...
class Steepest_Ascent_Hill_Climbing:
    '''Class for Steepest Ascent Hill Climbing search'''
    def __init__(self,s):  
        self.moves = 0
        self.print_nodes=[]
...
class Sideways_Move:
    '''Class for Sideways Move Hill Climbing search'''
    def __init__(self, s):
        initial_state = []
        self.start_node = Board()
        self.moves = 0
    ...
    def hill_climb(self):
        ...
        while True:                 #Infinite loop breaks when solution is found or not found
            ...
            if not better_heu and select_random_neighbours:           # To select a random neighbour
                ...
                count += 1
                self.moves += 1
            if not best_heu and not better_heu:                 # Return current node whether solution is found or not found
                return current_node
...
class Random_Restart_Steepest_Ascent:
    ...
    def generate_initial_state():
        '''Method to generate random initial state'''
        state = []
        for i in range(8):
    ...
    def get_random_restarts(self):                                     #
        '''To get the number of restarts used'''
        return Random_Restart_Steepest_Ascent.restart_used


class Random_Restart_Sideways_Move:
    '''Class for random restart sideways move'''
    ...
    def generate_initial_state():
        '''To generate random initial config'''
        start = []
        for i in range(8):
            start.append(Queen(random.randint(0, Board.get_size() - 1), i))
        return start

    def set_start_node(self, current_board):
        '''To set start node'''
