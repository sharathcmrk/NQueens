...
class Queen:
    '''Class with variables r(rows), c(columns) '''
    ...
    def attack_pairs(self, q):
        '''This function checks if there are any attacking pairs and returns a bool value'''
    ...
    def move_down(self, moves):
        '''Move the queen down along the column according to no of steps specified'''
...
class Board:
    ...
    def neighbour_node(self, initial_state):
        '''Method to find neighbour nodes'''
        count = 0
        for i in range(Board.board_size):
    ...
    def heu_diff(self, n):
        '''To find the diff btw current_node and neighbour node'''
        if (self.h < n.get_h()):
            return -1
        elif (self.h > n.get_h()):
            return 1
